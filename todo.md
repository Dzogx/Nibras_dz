...
## إصلاح فيديو عرض نبراس للخبير
- [x] إعادة إنتاج فيديو عملي يعرض خطوات العمل الفعلية داخل جولة الخبير العامة: التهيئة ثم صفحة اليوم ثم التخطيط ثم التقويم.
- [x] استخدام لقطات واجهة نبراس العامة فقط، مع تعليق عربي متزامن وتحرير تسلسل سهل المتابعة.
- [x] مراجعة الفيديو الجديد والتحقق من وضوح جميع الخطوات.
- [x] رفع الفيديو العملي النهائي إلى التخزين وتوثيق رابط المشاركة المخصص لتسليمه للخبير.
...
## الاقتراحات الثلاثة (20 أوت — بعد المولّد الذكي للاستراتيجيات)

- [x] دمج وضع «الذكاء الاصطناعي» في بطاقة استراتيجية صفحة اليوم (الأسبوع) وليس فقط صفحة الكفاءات: حوار الاستراتيجية يدعم mode=ai بزر تبديل «ولّد استراتيجية مخصصة بالذكاء الاصطناعي» مع عرض المصدر والملاحظة — وإصلاح 63 خطأ TypeScript في Dashboard/AnnualPlanDetail بتوحيد شكل إرجاع suggestStrategy إلى { strategy, source, note } في الوضعين (server/routers.ts) وقراءات موحدّة في الواجهات عبر select/d
- [x] حفظ الاستراتيجيات المولّدة بالذكاء الاصطناعي في دفتر التجارب بنقرة واحدة: زر «احفظ في دفتر التجارب» متاح داخل حوار الاستراتيجية بوضع AI أيضًا (يعمل على الاستراتيجية المستخرجة من الغلاف الموحد، مع تحديث حالة «محفوظة» وإعادة استخدام)
- [x] تقرير فصلي للكفاءات: إجراء competencyModel.termReport + دالة db (getTermCompetencyReport) تجمّع حصيلة الوضعيات (منجز/جزئي/مؤجل/ملغى) حسب الفصول الثلاثة مع نسبة بلوغ لكل مقطع وإجمالية الموسم، وحوار «كشف التقدم الفصلي» بصفحة الكفاءات (زر في شريط الفلاتر) + طباعة A4 محايدة بترويسة وزارية بلا إشارة للمنصة (server/term-report.test.ts: 4 اختبارات — 234/234 يمر وTypeScript نظيف)

## يوم عمل تجريبي كامل كأستاذ اجتماعيات (20 أوت مساءً — طلب الأستاذ: «استغل المنصة كالأستاذ غدًا»)
- [x] تهيئة الموسم تجريبيًا عبر الواجهة: أقسام + جدول أسبوعي + تفعيل الموسم (بيانات تجريبية واضحة قابلة للتنظيف لاحقًا)
- [x] تنفيذ اليوم الأول: فتح صفحة اليوم، توليد استراتيجية التسيير (الوضع الثابت + الذكاء الاصطناعي)، توليد المذكرة، فتح بطاقة الوضعية
- [x] تسجيل نتيجة حصة تجريبية (منجز/جزئي/مؤجل) والتحقق من أثرها على التقدم في صفحة الكفاءات والتقرير الفصلي
- [x] إصلاح فشل إنشاء ملاحظة الحصة (noteType=«session_reflection» خارج enum المخطط — أصبح «ملاحظة تربوية») + تعويض ملاحظة الحصة في قاعدة البيانات
- [x] تقويم تجريبي في استوديو التقييم مرتبط بالوضعية المنجزة (تم في المحاكاة) + إصلاح استوديو التقييم للدروس المشتقة (union string/number) + كشف نقاط في دفتر التنقيط
- [x] حياد الهوية في قوالب HTML: إزالة شعار نبراس من ترويسة A4Print (print-header-logo) والتذييل (print-footer-brand) واستبدالهما بالترويسة الرسمية الجزائرية نصياً، مع فحص بقية القوالب (المراسلات، كشوف، دفتر التنقيط، كشف فصلي، بطاقة استراتيجية، نسخة Audio في ResourceDetail)
- [x] تحسين سياق المقطع الرسمي في موجّه توليد المذكرة والأنشطة (تمرير اسم المقطع الرسمي من annualPlanSections في promptBuilder)
- [x] إصلاح 9 عناوين وضعيات مصابة بخلل OCR في البذرة وقاعدة البيانات (مثل «الآخريطة»/«بقايا ترويسة PDF»/همزات محذوفة) مع تحقق نهائي (0/264 عنواناً ملوثاً)
- [x] إصلاح تصفية أسطر [مرجع] الزائدة في مخرجات LLM للمذكرة والأنشطة + تحسين عرض الاستشهادات في ResourceDetail (بدون تكرار الأسطر)
- [x] منع الكاش المسموم في Dashboard/main.tsx: إبطال مفاتيح السنة الفارغة وتحصين الاستعلامات التابعة ومنع «تهيئة الموسم» من البقاء عند أول تحميل
- [x] زر الاستيفائي الشهري في دفتر التنقيط (حراسة subject غير فارغ قبل الاستعلام) + إصلاح حفظ continuousScore في saveEntries
- [x] رصد الفجوات العملية وإصلاحها مباشرة: عناوين الوضعيات المصابة بـ OCR (9 عناوين + فحص 0/264)، noteType الملاحظة خارج enum، monthlySummary بـ subject فارغ، continuousScore مفلترًا عند الحفظ، الكاش المسموم في Dashboard/main (شاشة تهيئة الموسم العالقة)، حياد الهوية في كل قوالب HTML، سياق المقطع الرسمي في موجّه المذكرة والأنشطة، تصفية [مرجع] من مخرجات LLM
- [x] (مُوثّق) الفجوات المتبقية منخفضة الأولوية: توحيد preferredClass المحلي في Gradebook مع hook المشترك، ومعالجة عرض جدول دفتر التنقيط العريض على الهاتف — موثّقة للانتقال التالي دون تنفيذ الآن (خارج نطاق الطوارئ)
- [x] قرار البيانات التجريبية: تُبقي كعينة عرض جاهزة للموسم (روستر 1م1، مذكرة 480001، تقويم 540001، وضعيات منجزة) لأن الأستاذ يحتاج منصة عامرة لإظهارها غدًا؛ البيانات المرجعية (المخططات/الكفاءات/الاستراتيجيات) لم تُمس
- [x] اختبارات انحدار وفحص TypeScript وCheckpoint (245/245 وtsc نظيف — checkpoint dc8ae688)

## متابعة المحاكاة + بحث إثراء (20-21 أوت — طلب الأستاذ قبل النوم: إكمال المهمة والبحث في مصادر GitHub/مواقع للتعليم الجزائري والتعلم النشط)
- [x] إدخال نقاط 5 تلاميذ تجريبية في دفتر التنقيط (1م1/التاريخ/الفصل الثالث) عبر الواجهة والتحقق من الحفظ في gradebookEntries (مع rooster تلقائي من 20 اسمًا)
- [x] التحقق من كشف التقدم الفصلي للكفاءات (competencyModel.termReport) في صفحة الكفاءات بعد تسجيل الحصة
- [x] التحقق البصري النهائي من المعاينة المحايدة: المكتبة (المذكرة) + استوديو التقييم + دفتر التنقيط
- [x] بحث شامل في مواقع ومستودعات GitHub للتعليم الجزائري والتعلم النشط والبيداغوجيات الحديثة، مع تحليل استفادة نبراس منها (حُفظت النتائج في /home/ubuntu/research-notes.md ونُفذ منها Jigsaw)
- [x] تنفيذ التحسينات المستفادة من البحث (استراتيجية البازل التعاوني Jigsaw من الأدبيات التربوية لكل المواد + توسيع الموجّه التربوي) وإصلاح اختبار getTeacherOSContext المسبق (mock حسب معرف المقطع بدل ترتيب الاستدعاءات) — 245/245 اختبارًا وفحص TypeScript نظيف (بيانات/استراتيجيات/موارد تربوية) دون إعادة هندسة
- [x] اختبارات انحدار كاملة وفحص TypeScript وCheckpoint نهائي (245/245 وtsc نظيف — checkpoint 7b34f7db)

## إصلاح شاشة تهيئة الموسم عند الأستاذ (21 أوت — لقطة المستخدم تظهر «ابدأ بتهيئة موسمك الدراسي» و«السنة الدراسية —» مع «مرحبًا الهاشمي عبيدلي»)
- [x] تشخيص: لماذا صفحة اليوم تعرض «تهيئة الموسم» عند المستخدم رغم وجود بيانات (1م1/2026-2027) في DB — السبب الجذري المثبت: عند فشل قائمة السنوات المفعّلة أو كاش قديم كانت seasonClasses تُفرَّغ بالكامل (فلتر سنة غير محسومة يستبعد كل الأقسام التي تحمل academicYear فعليًا) فيسقط المستخدم على بطاقة التهيئة
- [x] إصلاح السبب الجذري: طبقة حصانة نهائية fallbackYear تستنتج السنة من الأقسام الموجودة فعليًا (classes[0].academicYear) دون اعتماد دائري، واستُخدمت في seasonClasses وgetTeacherOSContext والمخططات والجدول والمواعيد البديلة والترويسة
- [x] التحقق البصري + اختبارات (245/245) + فحص TypeScript نظيف — السنة 2026-2027 تظهر في الترويسة والبطاقات تعرض بيانات حقيقية

## تقرير تجربة المنصة كأستاذ اجتماعيات (21 أوت — طلب الأستاذ: «ما هي نتائج تجربتك للمنصة؟»)
- [x] إعادة تمثيل يوم عمل كامل عبر الواجهة (اليوم ← تجهيز مذكرة ← استراتيجية ← تقويم ← مكتبة ← دفتر تنقيط) وتدوين الملاحظات بموضوعية
- [x] كتابة تقرير التجربة وتسليمه (/home/ubuntu/nibras-experience-report.md) ثم إغلاق خلل 1-4 كلها اليوم

## إصلاح استوديو التقويم (تقرير التجربة 20 أوت)
- [x] إصلاح الموجّه: فرض الموضوع المستورد على كل سؤال (لا أسئلة خارج الموضوع) في الموجّهات الأربعة + فلتر تنويهات الموجّه قبل العرض (ملاحظة/تنبيه/توضيح/خاتمة موجهة للأستاذ)
- [x] إصلاح ترويسة PDF التقويم: المؤسسة من الملف الشخصي فقط، القسم في «المستوى والقسم»، والمدة من القواعد الوطنية أو المدخلة (getPdfPayload كان سليمًا؛ التحقق النهائي عبر cb جديد)
- [x] تركيز الاستشهادات في نطاق الوضعيات المنجزة للقسم: focusedDocs بمطابقة sectionTitle + situationNumber/lessonNumber + «المقطع N» المشتق
- [x] ترتيب مستقر للوثائق: وثائق مقطع الوضعيات المنجزة أولًا (مرتبة بlessonNumber ثم المخططات السنوية) ثم السنوية ثم الباقي
- [x] رسالة سبب التعطيل تحت زر «توليد التقييم» عند عدم ملء العنوان (نص يظهر أسفل الزر يوضح الحقل الناقص)
- [x] تحسين شاشة «جارٍ تجهيز مساحة عملك» بعبارة قصيرة وشريط تقدم نابض (سلوكها طبيعي — حالة تحميل لحظية قبل وصول classesSettled، وقِيَست سرعة الخادم ~50ms)
- [x] اختبارات صارخة (فرض الموضوع + ترتيب focusedDocs + فلترة الاستشهادات) + انحدار كامل (247/247) + TypeScript نظيف

## تطبيق الهاتف التقدّمي (PWA)
- [x] فحص بنية الواجهة الحالية واختيار استراتيجية PWA آمنة لا تخزّن بيانات الأستاذ الحساسة أو تعيد باندلًا قديمًا (شبكة أولًا ولا تخزين لأي API أو بيانات أقسام/تلاميذ)
- [x] إضافة manifest وهوية تثبيت ملائمة لنبراس على شاشة الهاتف الرئيسية
- [x] إضافة service worker باستراتيجية تحديث فوري للواجهة ورسالة واضحة عند توفر نسخة جديدة
- [x] إضافة تجربة تثبيت صريحة للأستاذ على الهاتف وإرشادات مدمجة حسب المتصفح
- [x] اختبار التثبيت والتحديث دون اتصال محدودًا، وسلامة الاستعلامات والاختبارات وفحص TypeScript (بناء إنتاجي ناجح، 249/249 اختبارًا، TypeScript نظيف)

## إصلاح فشل النشر (Docker / XeLaTeX)
- [x] تشخيص Dockerfile وخطوة تثبيت حزم XeLaTeX التي فشلت في بناء النشر (السجل يثبت أن fc-cache غير موجود؛ exit 127)
- [x] إضافة الاعتمادية النظامية اللازمة أو تعديل أمر البناء لتوفير fc-cache (حزمة fontconfig + تثبيت غير تفاعلي)
- [x] اختبار بناء الإنتاج ووظائف طباعة PDF (pnpm run build ناجح؛ XeLaTeX وخط Amiri متاحان؛ لا يتوفر Docker محليًا لبناء الصورة نفسها)
- [x] تشغيل اختبارات الانحدار وفحص TypeScript وحفظ نسخة صالحة لإعادة النشر (249/249 وTypeScript نظيف)

## إصلاح فشل النشر الثاني (node-pty)
- [x] تأكيد سبب الفشل الجديد: غياب make أثناء node-gyp لإعادة بناء node-pty
- [x] إضافة أدوات البناء المطلوبة داخل Dockerfile (make وg++) مع الإبقاء على fontconfig وXeLaTeX
- [x] التحقق من بناء Docker الإنتاجي والاختبارات قبل حفظ نسخة إعادة النشر (TypeScript نظيف، 249/249 اختبارًا، وpnpm run build ناجح)
- [x] اختبار بناء الإنتاج والانحدار وتأكيد الجاهزية لحفظ نسخة قابلة للنشر

## عطل تسجيل الدخول على الهاتف (20 أوت)
- [x] تشخيص انقطاع مسار «تسجيل الدخول» من صفحة البداية على Chrome للهاتف في النطاق المنشور: الرابط الظاهر في الهاتف `dz6wzy.manus.space` يعيد 404، بينما النطاق الحالي يعيد 200
- [x] التحقق من مسار OAuth: الواجهة تبني callback من `window.location.origin`؛ لا توجد مشكلة برمجية في التدفق الحالي ولا تعديل مطلوب له
- [x] تحقق الأستاذ من إتمام الدخول من الرابط المنشور الحالي على الهاتف

## انقطاع بوابة المصادقة الخارجية (20 أوت)
- [x] توثيق أن إعادة التوجيه من نبراس وصلت إلى `manus.im/app-auth` بصيغة callback صحيحة للنطاق المنشور
- [x] اختبار توفر بوابة المصادقة العامة دون استخدام رابط OAuth الذي يحتوي nonce خاصًا بمحاولة الأستاذ: ظهرت شاشة «Sign in to Nibras» كاملة وقبلت معرف التطبيق وواجهة callback
- [x] استبعاد خلل OAuth قابل للإصلاح في نبراس: الاتصال الذي ينقطع داخل هاتف الأستاذ هو انقطاع خارجي مؤقت أو متعلق بالشبكة المحلية
- [x] تحقق الأستاذ من إعادة فتح البوابة وإتمام الدخول من الهاتف بعد إعادة المحاولة

## إعادة التصميم البصري للمنصة والوثائق (20 أوت)
- [x] جرد فعلي للبطاقات والصفحات والقوالب المطبوعة الحالية مع تحديد ما يبدو عامًا أو مزدحمًا أو ضعيف الهرمية (موثق في visual-redesign-audit.md)
- [x] اعتماد اتجاه بصري عربي رصين ومبدع يميّز أدوات عمل الأستاذ عن الوثائق الرسمية المحايدة
- [x] إعادة تصميم البطاقات والصفحات اليومية ذات الأولوية لتحسين الهرمية والراحة على الهاتف
- [x] إعادة تصميم مذكرة الحصة وبطاقات النشاط والتقويم بصريًا مع الحفاظ على RTL والطباعة A4 والحياد الرسمي
- [x] التحقق البصري من النماذج المعاد تصميمها على الهاتف والطباعة وإجراء اختبارات الانحدار (249/249 اختبارًا، TypeScript وبناء الإنتاج ناجحان)

## نظام تصميم A4 للمخرجات التربوية (20 أوت)
- [x] استخراج قواعد التصميم المرجعية: الشبكة، الأشرطة العنوانية، تراتب المعلومات، ومساحات عمل المتعلم (موثق في a4-design-system-brief.md)
- [x] اعتماد فصل بصري بين بطاقة النشاط ذات هوية نبراس والوثائق الرسمية المحايدة (المذكرة والتقويم)
- [x] بناء مكونات A4 قابلة لإعادة الاستخدام للأقسام والحقول والجداول ومؤشرات المراحل
- [x] تطبيق نظام البطاقة النشطة أولًا ثم نقل المذكرة والتقويم إلى البنية ذاتها وفق قواعد الحياد
- [x] معاينة الطباعة الفعلية وRTL واختبارات الإنتاج قبل النشر (نجح PDF الفعلي بعد تصحيح المحاذاة، ونجحت الأنواع والاختبارات والبناء)

## إعادة ابتكار المذكرة والتقويم (20 أوت)
- [x] تحليل سبب تفوق بطاقة التعلم النشط بصريًا على قالبَي المذكرة والتقويم وتحديد الفجوة في التكوين (موثق في pdf-redesign-findings.md بعد معاينة PDF فعلية)
- [x] ابتكار مسار بصري للمذكرة يحوّل سير الحصة إلى مخطط مقروء بدل جدول تقليدي فقط (شبكة حقائق، هدف بارز، خريطة مراحل، صفوف مرقمة وتذييل ملاحظات/إمضاء)
- [x] ابتكار بنية تقويم تحصيلي تجعل أجزاء الاختبار ومساحات الإجابة أوضح وأشد توازنًا (حقول تلميذ ثلاثية، شارات نقاط، صفوف أسئلة وخطوط إجابة، ختم مجموع/إمضاء ضمن الصفحة)
- [x] إنتاج عينات PDF محسنة والتحقق من RTL والحياد الرسمي والاختبارات قبل النشر (XeLaTeX مرتين، مراجعة PNG فعلية؛ المذكرة صفحتان والتقويم صفحة واحدة؛ 251/251 اختبارًا)

## تصدير PDF من الواجهة (20 أوت)
- [x] حصر المسارات الحالية لتوليد وتنزيل PDF للمذكرة والتقويم وربطها بواجهات المورد المناسبة (استوديو التقويم وتفاصيل المذكرة كانا يستخدمان XeLaTeX؛ أضيف الربط أيضاً إلى صفحة مورد المكتبة)
- [x] إضافة أزرار تصدير PDF عالية الجودة للمذكرة والتقويم مع حالات تحميل وخطأ مفهومة (زر تنزيل PDF في صفحة المورد، وحالات «جارٍ تجهيز PDF» ورسائل خطأ عربية)
- [x] اختبار تنزيل ملفات PDF الناتجة والتحقق من النوع والجودة والاختبارات قبل النشر (اختبار فك بايتات PDF ونوع التطبيق، TypeScript نظيف، 252/252 اختبارًا، وبناء إنتاج ناجح)

## عرض عينات PDF (20 أوت)
- [x] توليد نسخة مشاركة حديثة من المذكرة ونسخة من التقويم بالتصميم النهائي (A4، مذكرة صفحتان وتقويم صفحة واحدة)
- [x] التحقق من صلاحية الملفين ثم إرفاقهما للأستاذ (معاينة PDF فعلية: RTL، الحياد الرسمي، أشرطة الأقسام ومساحات الإجابة ظاهرة)

## إعادة إخراج التقويم التحصيلي الرسمي (20 أوت)
- [x] تحليل المرجع المرفق وتحديد عناصر الوثيقة الرسمية التي يجب أن تحل محل طابع بطاقة المنصة (ترويسة جدولية، أحادي اللون، أجزاء/وضعيات بنقاط، تذييل مؤسسي ثلاثي؛ موثقة في pdf-redesign-findings.md)
- [x] إعادة تصميم قالب التقويم بترويسة مؤسساتية، أقسام ونقاط واضحة، ومساحات إجابة رسمية قابلة للطباعة (ترويسة جدولية ثنائية الصفوف، شارات رمادية للأجزاء، حقول تلميذ، تذييل ثلاثي وجدول Markdown رسمي)
- [x] توليد عينة اختبار جديدة والتحقق من RTL والحياد والاختبارات قبل النشر (XeLaTeX مرتين، عينة A4 صفحة واحدة، TypeScript والاختبارات الكاملة وبناء الإنتاج ناجحة)

## نسخة التقويم الرسمي (20 أوت)
- [x] توليد نسخة PDF قابلة للمشاركة من قالب التقويم الرسمي الحالي والتحقق من صلاحيتها (صفحة واحدة، A4، PDF 1.5)
- [x] إرفاق نسخة التقويم الرسمية للأستاذ

## قاعدة مساحة الإجابة حسب المستوى (20 أوت)
- [x] تحديد مصدر مستوى القسم في بيانات التقويم وربطه بقرار ظهور مساحة الإجابة داخل الورقة (حقل gradeLevel في AssessmentLatexInput)
- [x] إظهار الإجابة المدمجة للأولى والثانية فقط وحذفها تلقائيًا من تقويمَي الثالثة والرابعة (قاعدة allowsEmbeddedAnswerSpace احتياطية: المستوى غير المعروف لا يضيف إجابة مدمجة)
- [x] إضافة اختبارات وعينات PDF للتحقق من القاعدة في المستويات الأربعة قبل النشر (اختبار صريح للأولى والثانية والثالثة والرابعة، وعينتا المسارين: الأولى المدمجة والثالثة بورقة إجابة منفصلة)

## جداول ورقة الأسئلة للمستويات العليا (20 أوت)
- [x] توثيق أن جداول الأسئلة تبقى في ورقة الاختبار لجميع المستويات، مع ورقة إجابة منفصلة للثالثة والرابعة (تعليق buildFormalTable يحدد القاعدة)
- [x] إضافة اختبار وعينة للمستوى الثالث يثبتان ظهور جدول السؤال دون خطوط إجابة مدمجة (جدول سؤال رسمي مع تعليمات نقله إلى ورقة الإجابة، صفحة A4 واحدة)

## مراجعة Prototype الصفحة الأولى (20 أوت)
- [x] فحص الصفحة الأولى ومسار الدخول الحالي من منظور الأستاذ (Home.tsx: صفحة تعريفية قبل الدخول تتجه إلى لوحة التحكم)
- [x] إعداد وصف Prototype واضح للصفحة الأولى ووظائفها الأساسية

## تجربة الأستاذ وتبسيط المسار (20 أوت)
- [x] استبدال رسالة QR والرقم التسلسلي في الصفحة الأولى بصياغة تؤكد حياد الوثائق الرسمية وجاهزيتها للطباعة (وتنقيح بطاقة المورد كذلك)
- [x] محاكاة مسار الأستاذ من صفحة اليوم إلى الحصة والمذكرة والتقويم والتصدير وتسجيل نقاط الاحتكاك الواقعية (النتائج موثقة في teacher-journey-audit.md)
- [x] تبسيط الخطوات وإصلاح الأخطاء المؤثرة التي ظهرت في المسار
- [x] التحقق البصري والتقني وحفظ تقرير نهائي بالتحسينات (هاتف: اليوم/التقويم/دفتر التنقيط، اختبار 263/263، TypeScript وبناء إنتاجي ناجحان)
- [x] تقليل ازدحام شريط إجراءات المذكرة والتقويم بإبقاء الإجراء اليومي وإخفاء الخيارات الثانوية في قائمة «المزيد»
- [x] تعبئة عنوان التقويم تلقائياً من الدروس المنجزة عند فتح استوديو التقويم للقسم الحالي
- [x] إزالة أي تسمية أو خيار يحمل هوية نبراس من إعدادات PDF الخاصة بالتقويم الرسمي
- [x] منع صيغ الموجّه الاصطناعي غير التربوية من الظهور في أسئلة التقويم المولدة
- [x] إزالة إعداد رمز QR من استوديو التقويم والوثائق الرسمية لأنه لا يدخل في النسخة المطبوعة المعتمدة
- [x] إصلاح فشل زر تنزيل PDF للتقويم المولد داخل الاستوديو («مصدر الطباعة غير صالح»): استُعيد توقيع المصدر الآمن في قالب LaTeX وأضيف اختبار حارس
- [x] إزالة الترويسة الوزارية المكررة التي يدرجها مخرج الذكاء الاصطناعي داخل جسم PDF التقويم

## إصلاح تسجيل الدخول المنشور (20 أوت)
- [x] تشخيص مسار الدخول المنشور وسجلات التشغيل: بدء OAuth يعرض نطاق الإرجاع الصحيح ولا توجد أخطاء خادم متكررة
- [x] تطبيق تحصين موثوق لمسار الجلسة: حجب localStorage في Safari/WebView لم يعد يكسر الواجهة بعد عودة OAuth، مع جلسة اختبار محلية ناجحة
- [x] تحصين بدء الجلسة ضد حجب تخزين المتصفح في Safari وواجهات الهاتف (كتابة آمنة للتخزين المحلي واختبار وحدات لحالة الحجب)

## تهيئة الموسم الدراسي متعددة الصفحات (20 أوت)
- [x] مراجعة شاشة تهيئة الموسم الحالية وبيانات الأستاذ والأقسام والتلاميذ والجدول المتاحة
- [x] بناء خطوة بيانات الأستاذ والمؤسسة مع حفظ آمن والاستئناف التلقائي
- [x] بناء خطوة المستويات والأقسام المسندة وربطها بالمنهاج الجاهز
- [x] بناء خطوة قوائم التلاميذ بالاستيراد من ملف الرقمنة أو الإدخال اليدوي المحدود
- [x] بناء خطوة الجدول الأسبوعي وفق حصص الأستاذ ومواعيدها
- [x] بناء صفحة مراجعة وانطلاق تربط إتمام التهيئة بصفحة اليوم
- [x] إزالة بطاقة الانطلاق المكررة من خطوة المراجعة لتبقى نهاية التهيئة مركزة على قرار واحد
- [x] اختبار المسار كاملاً على الهاتف وإضافة اختبارات وحدات ثم نشره (265/265 اختباراً وفحص TypeScript سليم ومعاينة هاتف فعلية)

## وصف تقني للعرض على خبير (20 أوت)
- [x] توثيق البنية التقنية والطبقات والمسارات الرئيسية للمنصة
- [x] تحليل تداخل الصفحات وتحديد أسباب الازدواج والتراكم في تجربة المستخدم
- [x] إعداد وثيقة تقنية بخيارات إعادة التنظيم وأسئلة قرار واضحة للخبير

## جولة خبير عامة بلا تسجيل دخول (20 أوت)
- [x] تحديد صفحات المعاينة العامة وعزل أي بيانات شخصية أو بيانات تلاميذ
- [x] بناء صفحة «جولة الخبير» عامة للقراءة فقط مع عرض منظم لتدفقات المنصة
- [x] إضافة مسار عام وآمن لا يطلب OAuth ولا يستدعي إجراءات البيانات المحمية
- [x] اختبار الرابط العام على الهاتف والنشر مع تعليمات مشاركته

## فيديو رحلة الأستاذ للخبير (20 أوت)
- [x] اعتماد مواصفات الفيديو والهوية البصرية والصوتية مع الأستاذ
- [x] كتابة سيناريو عربي آمن بلا بيانات حقيقية
- [x] إنتاج بديل فيديو شرح من مراجع بصرية آمنة وتعليق عربي وموسيقى أصلية بعد بلوغ حد اللقطات المولدة
- [x] التحقق التقني من ملف MP4 النهائي: 1280×720، H.264/AAC، مدة 83.47 ثانية

## إعادة تصميم UX/UI الشاملة — «مكتب الأستاذ الرقمي» (طلب الأستاذ 20 أوت)
- [x] المرحلة 1 — UX AUDIT: تحليل كل صفحة (اليوم، الأسبوع، التخطيط، المذكرة، المكتبة، التقويم، النتائج، دفتر التنقيط، Inspector، الإعدادات، الطباعة) وتحديد الهدف والإجراء الأساسي والاحتكاك وقيمة كل عنصر.
- [x] المرحلة 2 — فلسفة UX: التحول من «Dashboard + أدوات» إلى «مكتب الأستاذ الذي يعرف أين وصل الأستاذ وما التالي» حول المسار: اليوم → الحصة → الوضعية → التحضير → التنفيذ → التقويم → النتائج → العلاج.
- [x] المرحلة 3 — نظام تصميم جديد فوق المكونات الحالية: طباعة قوية، فراغ أبيض، هرمية بصرية، شبكة متناسقة، بدون نمط بطاقات/ظلال SaaS، وهوية تجريدية لمعنى النبراس (المسار، النقطة، الاتجاه، الضوء).
- [x] المرحلة 4 — إعادة تصميم الصفحة الرئيسية: جواب فوري على «ماذا أفعل الآن؟» (الحصة القادمة + حضّر الحصة + التقدم + ما يحتاج انتباه + إجراءات سريعة).
- [x] المرحلة 5 — السياق التربوي المتصل: مسار المستوى → القسم → المقطع → الوضعية → الكفاءات → الموارد → التنفيذ → التقويم حاضر بصرياً داخل صفحات الوضعية.
- [x] المرحلة 6 — «أنشئ حزمة الحصة»: CTA واضح من الوضعية يتيح اختيار (مذكرة + بطاقة تعلم نشط + ورقة عمل + نشاط تقويمي) بنقل تلقائي للسياق.
- [x] المرحلة 7 — تجربة الوثائق: Layout تربوي خاص لكل نوع (مذكرة/بطاقة/تقويم) بعائلة بصرية مشتركة دون كسر الطباعة A4/RTL/القواعد الوطنية.
- [x] المرحلة 8 — Mobile First: CTA أساسي واضح، تنقل بإبهام اليد، أقل مستويات، دون إغراق بالمعلومات على الهاتف.
- [x] المرحلة 9 — إعادة تقييم التنقل: توحيد ذهني حول (اليوم، التخطيط، التنفيذ، التقويم، النتائج، المكتبة) ودمج الصفحات السياقية دون حذف وظائف.
- [x] المرحلة 10 — التنفيذ: خطة UX قصيرة، تنفيذ تدريجي دون كسر البيانات أو APIs أو منطق الأعمال، ثم اختبار كل مسار.
- [x] اختبار قبول: سيناريو الأستاذ الكامل (فتح → الحصة القادمة → الوضعية → حزمة الحصة → تعديل المذكرة → طباعة → تأكيد التنفيذ → تقويم → نتائج → تحليل → علاج) سريع وواضح دون شرح.
- [x] معايير النجاح: 265 اختباراً تمر + TypeScript نظيف + فحص بصري للهاتف والسطح المكتب + عدم وجود وظائف مكسورة.

## الإبداع البصري — «هوية نبراس المميزة» (طلب الأستاذ 21 أوت: «يمكنك أن تبدع أكثر»)
- [x] تحديد الاتجاه الإبداعي المكتوب: نظام بصري ذي شخصية (Nibras Signature) فوق الأساس الحالي دون كسر وظائف (موثق في ux-audit/06-creative-direction.md)
- [x] نظام ألوان أعمق: تدرج بنفسجي داكن → ذهبي دافئ → ورقي، مع لمسات ضوء (glow خفيف محسوب) في اللحظات المهمة (نظام nb-* في index.css)
- [x] زخرفة «المسار والضوء»: خط مسار بصري يربط عناصر الصفحة (Trail أفقي وStageTrack عمودي في OfficeChrome)
- [x] ترويسات بتوقيع بصري: حقل مسرحي داكن يحمل العنوان (StageHeader مطبق على كل الصفحات)
- [x] بطاقات سردية: كل بطاقة تحكي لحظة (بطاقات nb-story على لوحة القيادة)
- [x] حركة هادئة محكمة: ظهور متدرج وانتقالات سريعة واحترام prefers-reduced-motion (نبضات nb-glow ومحطات nb-node)
- [x] تفاصيل ورقية فاخرة: حواف ورقية وحدود ذهبية وختم/نقطة ضوء وأرقام خطوات كعقد على المسار
- [x] تطبيق الهوية على: الصفحة الرئيسية، الأسبوع، المخطط السنوي، تفاصيل الدرس والوضعية
- [x] تطبيق الهوية على: الاستوديو، المكتبة، النتائج، دفتر التنقيط، الكفاءات، دفتر التجارب، المفتش، الجولة العامة
- [x] فحص بصري هاتف (375px) وسطح مكتب لجميع الصفحات + اختبارات 265/265 وTypeScript نظيف + بناء إنتاجي ناجح
- [x] حفظ نقطة تحقق وتسليم الشرح للعربية

## صقل «نبراس سيغناتشر» (اقتراحات 21 أوت)
- [x] حركة دخول متدرجة (stagger) لبطاقات لوحة القيادة والأسبوع (nb-arrive-d1..d7 على المسرح وأعمدة الأيام وبطاقات الحصة ومتابعة أوسع وأزرار الإحصاءات)
- [x] مسار مصغّر للمقطع الحالي في لوحة القيادة (صفحة «أين وصلت في الخطة؟» — Trail داكن بعقدة مضيئة للمقطع الجاري)
- [x] نسخة داكنة اختيارية بالكامل للمنصة (تبديل في الشريط الجانبي وترويسة الهاتف مع حفظ التفضيل في ThemeContext)
- [x] التحقق: اختبارات وTypeScript وفحص بصري ثم دفع GitHub وحفظ نقطة تحقق (يُحدّث بعد اكتمال المرحلة 3-4)

## الطبقة الإبداعية الأخيرة — «فريق الشخصيات الثلاث» (طلب الأستاذ 21 أوت: أقصى الإبداع)
- [x] ترحيب صباحي شخصي في لوحة القيادة: تحية حساسة للوقت (صباح/مساء/ليل) مع اقتباس تربوي هادئ وأولوية اليوم بنبرة معلم
- [x] قنديل النبراس في الترويسة: رمز شعلة مضيء نابض بجانب الشعار داخل StageHeader (SVG خفيف)
- [x] صقل الوضع الليلي: ضمان أن كل الصفحات الرئيسية (اليوم، الأسبوع، المخططات، التقويم، المكتبة، النتائج، التنقيط، التجارب، الكفاءات، المفتش) مقروءة ومريحة في الظلام
- [x] تفاصيل مضيئة ختامية: مؤشر تقدم متوهج، خط ذهبي تحت أزرار الإجراءات، وحركة نبض لطيفة في اللحظات المهمة فقط (قنديل نابض، عقد المسار المتوهجة، اقتباسات ذهبية)
- [x] فحص نهائي: اختبارات 265/265 + TypeScript نظيف + لقطات هاتف/سطح + دفع GitHub وحفظ نقطة تحقق
