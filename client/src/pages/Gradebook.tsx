...
/** زر الطباعة متاح أيضًا من قائمة الروفستر (روستر فارغ + dirty) عبر prop بديل. */
function RosterPrintButton({ roster, className, subject, term }: { roster: unknown; className: string; subject: string; term: number }) {
  const [printOpen, setPrintOpen] = useState(false);
  const hasRows = Array.isArray(roster) && roster.length > 0;
  const rows = (Array.isArray(roster) ? roster : []) as Row[];
  const TERM_LABELS: Record<number, string> = { 1: "الفصل الأول", 2: "الفصل الثاني", 3: "الفصل الثالث" };
  const termLabel = TERM_LABELS[term] ?? `الفصل ${term}`;
  const print = () => {
    if (!hasRows) {
...
<div class="hdr"><table class="small"><tr>
<td><div>الجمهورية الجزائرية الديمقراطية الشعبية</div><div>وزارة التربية الوطنية</div></td>
<td class="center"><div class="small" style="font-weight:bold;">دفتر التنقيط — كشافة التلاميذ</div><div>${className} — ${subject}</div><div>${termLabel} — السنة الدراسية</div></td>
<td class="center"><div>عدد التلاميذ: ${rows.length}</div></td>
</tr></table></div>
<table class="sheet"><thead><tr>
<th>الترتيب</th><th>الاسم واللقب</th><th>الانضباط والمواظبة<br>/10</th><th>إنجاز الأنشطة<br>/10</th><th>التقويم المستمر<br>/20</th><th>الفرض الكتابي<br>/20</th><th>الاختبار الفصلي<br>/20</th><th>ملاحظات نوعية</th>
...
<div class="sig"><div class="small">إمضاء الأستاذ(ة)</div><div class="small">إمضاء مدير(ة) المؤسسة</div></div>
  ...
  return (
    <>
      <Button variant="outline" disabled={!hasRows} onClick={print}>
        <Printer className="ml-2 size-4" />
        طباعة الدفتر
      </Button>
      <Dialog open={printOpen} onOpenChange={setPrintOpen}>
        <DialogContent className="max-w-4xl w-[95vw] max-h-[90vh] overflow-hidden p-0 flex flex-col">
          <DialogHeader className="px-5 pt-4 pb-2">
            <DialogTitle className="text-right">{`معاينة كشافة التنقيط — ${className} · ${subject} · ${termLabel}`}</DialogTitle>
          </DialogHeader>
          <div className="flex-1 min-h-0 px-5 pb-4 flex flex-col gap-3">
            <iframe
              title="كشافة التنقيط للمعاينة"
              srcDoc={buildPrintHtml()}
              className="w-full flex-1 min-h-[50vh] border rounded bg-white"
              onLoad={(e) => printFromFrame(e.currentTarget)}
            />
            <Button onClick={() => printFromFrame(document.querySelector<HTMLIFrameElement>('iframe[title="كشافة التنقيط للمعاينة"]'))}>
              <Printer className="ml-2 size-4" />
              طباعة الآن (Ctrl+P)
            </Button>
          </div>
