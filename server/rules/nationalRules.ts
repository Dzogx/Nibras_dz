...
/**
 * ضوابط جودة اللغة العربية في كل مخرَج نصي: يُحقن في تعليمات مولّد المذكرة.
 * يغطي الأخطاء الإملائية المتكررة في مخرَجات LLM بالكتابة العربية (التاء المربوطة،
 * تكرار الأحرف المزدوجة، سلامة الجمل، وحدة السؤال).
 */
export const ARABIC_QA_RULES = `
ضوابط جودة الصياغة العربية (ملزمة، راجعها قبل التسليم):
- الأسماء المعدودة تُكتب بالتاء المربوطة: «المرحلة الثالثة»، «الخطوة الأولى»، «الفقرة الثانية» (لا «المرحلة الثالث»).
- لا تكرر حرفاً مزدوجاً: لا «خخطوات» ولا «تتناززل»؛ اكتبها «خطوات» و«تنازل».
- كل جملة سليمة نحويًا ومتصلة: لا تدمج سؤالين في جملة واحدة ولا تترك نصاً مكسوراً مثل «يتبعها العالم لأسئلة الماضي؟» — اكتب سؤالاً واحداً واضحاً.
- علامات الترقيم العربية: استخدم الفاصلة العربية (،) والنقطتين (:). لا تضع نقطة داخل كلمة.
- المدة الزمنية بصيغة «55 دقيقة (حصة واحدة)» عند وجود مدة محددة، لا «ساعة واحدة» إلا إذا حُددت ساعة كاملة.
- الأرقام تُكتب بأرقام عربية هندية أو لاتينية موحدة داخل الوثيقة نفسها، لا تخلط النظامين.
- راجع الناتج نهائياً واقرأه جملة جملة قبل إرجاعه.
- اذكر عناصر الحصة (المقطع والوضعية والكفاءة) بأسمائها الرسمية كما وردت في المخطط الصففي للمقطع المعني، ولا تستبدلها بعبارات عامة مثل «حسب التدرجات السنوية الرسمية» أو «المقطع الأول» دون اسمه.
- لا تكتب أي خطوة أو عنواناً عاماً مثل «...»؛ كل عنوان يجب أن يكون محدداً بالموضوع الفعلي للوضعية.`;
